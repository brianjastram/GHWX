...
#define AM2315_WARM_UP_TIME_MS 100
...
public:
    AM2315C_Sensor(TwoWire* wire = &Wire, int8_t powerPin = -1, uint8_t i2cAddress = 0x38);
    bool setup() override;
    bool addSingleMeasurementResult(int varNum, float result);
    ...
    ~AM2315C_Sensor() {}
protected:
    Adafruit_AM2315 _am2315;  // Declare _am2315 as an Adafruit_AM2315 object
};

class AOSongAM2315 : public Sensor {
public:
    AOSongAM2315(TwoWire* wire = &Wire, int8_t powerPin = -1)
        : Sensor("AOSongAM2315", 2, 100, 500, 500, powerPin, -1, AM2315_I2CADDR),
          _am2315(new Adafruit_AM2315(wire)) {}
    ...
    bool isConnected() {
        float temp, hum;
        return _am2315->readTemperatureAndHumidity(&temp, &hum);
    }

    bool readTemperatureAndHumidity(float* temperature, float* humidity) {
        if (_am2315->readTemperatureAndHumidity(temperature, humidity)) {
            Serial.print("AM2315C at I2C address: ");
            Serial.println(AM2315_I2CADDR, HEX);
            delay(2000); // Delay to prevent too frequent reads
            return true;
        }
        else {
            Serial.print("AM2315C read error at I2C address: ");
            Serial.println(AM2315_I2CADDR, HEX);
        }
        return false;
    }
