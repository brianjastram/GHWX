...
AM2315C::AM2315C(int8_t powerPin, TwoWire* wire)
    : Sensor("AM2315C", 2, 100, 500, 500, powerPin, -1, AM2315C_ADDRESS),
      _humidity(0), _temperature(0), _humOffset(0), _tempOffset(0),
      _status(0), _lastRequest(0), _lastRead(0), _wire(wire) {}
...
int AM2315C::convert()
{
  ...
  raw <<= 4;
  raw += (_bits[3] >> 4);
  _temperature = raw * 9.5367431640625e-5;   // ==> / 1048576.0 * 100%;
  ...
  raw <<= 8;
  raw += _bits[5];
  _humidity = raw * 1.9073486328125e-4 - 50;  //  ==> / 1048576.0 * 200 - 50;
...
bool AM2315C::addSingleMeasurementResult(void) {
    // Read sensor data
    if (read() != AM2315C_OK) {
        return false;  // Return false if the reading fails
    }

    // Store the values in sensorValues array
    sensorValues[0] = getTemperature();
    sensorValues[1] = getHumidity();
